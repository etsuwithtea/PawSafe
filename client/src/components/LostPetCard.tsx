    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden flex flex-col h-full" style={{ fontFamily: 'Poppins, Anuphan' }}>
      <div className="relative w-full h-48 bg-gray-200 overflow-hidden shrink-0">
        {lostPet.images && lostPet.images.length > 0 ? (
          <>
            <img
              src={lostPet.images[currentImageIndex]}
              alt={lostPet.name}
              className="w-full h-full object-cover"
            />
            {lostPet.images.length > 1 && (
              <>
                <button
                  onClick={() => setCurrentImageIndex((prev) => (prev === 0 ? lostPet.images.length - 1 : prev - 1))}
                ...
                <button
                  onClick={() => setCurrentImageIndex((prev) => (prev === lostPet.images.length - 1 ? 0 : prev + 1))}
                ...
                <div className="absolute bottom-2 right-2 bg-black bg-opacity-70 text-white px-2 py-1 rounded text-xs font-medium">
                  {currentImageIndex + 1}/{lostPet.images.length}
                </div>
              </>
            )}
          </>
      ...
      <div className="p-5 flex flex-col grow">
        ...
        <div className="flex gap-2 mt-auto">
          ...
          {user?._id !== lostPet.contactUserId && (
            <button 
